...
class Emails:
	...
	def emails_list(self):
		make_request = self.session.get(e.options[self.env] + e.endpoints_emails['email'])
	...
	def emails_delete_unconfirmed(self, *args):
		try:
			if "@" in args[0]:
				...
				url = e.options[self.env]+e.endpoints_emails['email_delete']
	...
	def emails_add(self,*args):
		try:
			'@' in args[0]
			link = e.options[self.env]+e.endpoints_emails['email']
			data = {'email': args[0]}
